...
class LoginGithubViewModel extends ChangeNotifier {
  ...
  LoginGithubViewModel() : _githubAuth = GitHubAuth((info) {});
  ...
  void _handleError(OAuthException error) {
    _errorOccurred = true;

    switch (error.errorType) {
      case AuthenticationErrorType.userCancelled:
        errorMessage = "It seems like you cancelled the login process. [Error Type: userCancelled]";

      case AuthenticationErrorType.noTokenReceived:
        errorMessage = "Github did not return a token. Please try again. If the problem persists, please contact us. [Error Type: noTokenReceived]";

      case AuthenticationErrorType.noUserCodeReceived:
        errorMessage = "Github did not return a user code. Please try again. If the problem persists, please contact us. [Error Type: noUserCodeReceived]";

      case AuthenticationErrorType.serverError:
        errorMessage = "An error occurred on the server. Please try again. [Error Type: serverError]";

      case AuthenticationErrorType.invalidGrant:
        errorMessage = "GitHub reported an invalid grant. This is likely due to errors within our confidential intermediary server. Please try again. If the problem persists, please contact us. [Error Type: invalidGrant]";

      case AuthenticationErrorType.badVerificationCode:
        errorMessage = "GitHub reported a bad verification code. Please try again. If the problem persists, please contact us. [Error Type: badVerificationCode]";
      case AuthenticationErrorType.loginProcessNotActive:
        errorMessage = "This looks like an internal error on our end. Please contact us. [Error Type: loginProcessNotActive]";
      case AuthenticationErrorType.maxRetriesReached:
        errorMessage = "The maximum number of retries has been reached. Check your internet connection and try again . [Error Type: maxRetriesReached]";
    }
    notifyListeners();
